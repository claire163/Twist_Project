...
df = pd.read_csv(args.data, skiprows=args.header)

# regression results
if len(df.columns) == 4:
    plt.plot(df['y'], df['mu'], 'o', ms=6, color='grey', alpha=0.7)
    for p in ['cschrimson', 'c1c2', 'cheriff']:
        plt.plot(df[df['name']==p]['y'], df[df['name']==p]['mu'], 'o', ms=8,
                color=parents[p], alpha=0.8)
        plt.margins(0.02)
        plt.xlabel('measured ' + args.name)
        plt.ylabel('predicted ' + args.name)
        lg = plt.legend(('chimeras', 'CsChrimR', 'C1C2', 'CheRiff'), loc='best')
